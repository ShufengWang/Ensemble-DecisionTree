...
int main()
{
	...
	sprintf(address, "%s", "models/adaboost.txt");
	...
	//		cv::Rect gt_rect(posGT[i][j].x-0.33*posGT[i][j].width+left, posGT[i][j].y-0.33*posGT[i][j].width+top, 
	//			1.66*posGT[i][j].width, 1.66*posGT[i][j].width);
	...
	//			cout<<" *";
	//		if(i<posSampNum){
	//			sprintf(address, "%s%d%s", "inria_adaboost_pos/", i, ".txt");
	//	    	response.at<int>(i,0) = 1;
	//	    	posCount++;
	//		}else{
	//			sprintf(address, "%s%d%s", "inria_adaboost_neg/", i-posSampNum, ".txt");
	//			response.at<int>(i,0) = -1;
	//			negCount++;
	//		}
 ...
 //       cv::Mat featFreq(featIdx.size(), 1, CV_32SC1, cv::Scalar(0));
	...
	//	cv::namedWindow("freqTemps");
	//	cv::imshow("freqTemps", freqTemps);
	//	cv::waitKey(50);
	//	cv::imwrite("freqTemps.png", freqTemps);
	...
	//					cv::Rect gt_rect(negGT[i][k].x-0.33*negGT[i][k].width, negGT[i][k].y-0.33*negGT[i][k].width, 
	//		                        		1.66*negGT[i][k].width, 1.66*negGT[i][k].width);
	...
	//					sprintf(address, "%s%d%s", "inria_adaboost_neg/", newSampStaNo, ".jpg");
	//					imwrite(address, neg_patch);

	//					vector<float> neg_dscrpt;
	//					generate_feature(neg_patch, tloc, gridSize, temps, featIdx, neg_dscrpt);
	//					sprintf(address, "%s%d%s", "inria_adaboost_neg/", newSampStaNo, ".txt");
	//					fileIn.open(address);
	//					for(int r=0; r<neg_dscrpt.size(); r++)
	//						fileIn<<neg_dscrpt[r]<<"\n";
	//					fileIn.close();
	...
	//FILE *fp_det;
	//fp_det = fopen("adaboost_mod/det.txt", "wb");
	//for(float thr=20; thr>=-50; thr-=2){
	//	float totalObjNum = 0;
	//	float detectObjNum = 0;
	//	float falsePosNum = 0;

	//	string resultAddress = "adaboost_res/";
	//	evaluate(resultAddress, testFile, testGT, false, 0.7, thr, totalObjNum, detectObjNum, falsePosNum);
