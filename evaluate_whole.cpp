	...
	bool nonMaxSup, float matchThr, float rspnThr, float &totalObjNum, float &detectObjNum, float &falsePosNum)
{
	...
	for(int i=0; i<fileName.size(); i++){
		...
		for(int j=0; j<groundTruth[i].size(); j++){
			if(detection.size()>0){
				cv::Rect gt;
				/*gt.x=groundTruth[i][j].x+0.5*groundTruth[i][j].width-0.333*groundTruth[i][j].height;
				gt.y=groundTruth[i][j].y-0.167*groundTruth[i][j].height;
				gt.width=0.667*groundTruth[i][j].height;
				gt.height=1.334*groundTruth[i][j].height;
				*/
				gt.x=groundTruth[i][j].x+0.5*groundTruth[i][j].width-0.3125*groundTruth[i][j].height;
				gt.y=groundTruth[i][j].y-0.125*groundTruth[i][j].height;
				gt.width=0.625*groundTruth[i][j].height;
				gt.height=1.25*groundTruth[i][j].height;
